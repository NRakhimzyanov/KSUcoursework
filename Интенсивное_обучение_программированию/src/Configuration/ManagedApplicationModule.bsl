...
АСИНХ Процедура ОпроситьИмяПользователя()
	...
	Если ДатаРождения = Дата('00010101') ИЛИ ДатаРождения > ТекущаяДата() Тогда
...
Процедура ВыбратьВариантПоздравленияПоздравить()	
	...
	Если Месяц(ДатаРождения) <> Месяц(ТекущаяДата()) ИЛИ (ДельтаДень > 5 И ДельтаДень < -5) Тогда
		ВариантПоздравления = "";
...
Сообщение.Текст = СтрШаблон("На дату %1 остаток металла на складе ""%2"": %3 шт.",НашаДата,НазваниеСклада,КоличествоМеталла);
Сообщение.Сообщить();
