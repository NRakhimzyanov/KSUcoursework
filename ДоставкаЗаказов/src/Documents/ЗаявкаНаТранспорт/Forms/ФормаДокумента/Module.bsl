...
Процедура ТранспортНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ТранспортныйОтделКлиент.ОткрытьФормуВыбора(ТранспортСБригадой, "Справочник.ТранспортныеСредства.ФормаВыбора", Элементы.Транспорт, "Наименование")
...
Функция ТранспортСБригадой(Дата)
	...
	|ИЗ
	|	РегистрСведений.СоставБригады.СрезПоследних(, Период >= &НачалоПериода И Транспорт НЕ В
	|		(ВЫБРАТЬ
	|			СостояниеДоставкиСрезПоследних.Транспорт
	|		ИЗ
	|			РегистрСведений.СостояниеДоставки.СрезПоследних(, Период >= &НачалоПериода И СостояниеЗаказа <> &СостояниеЗаказа) КАК
	...
	ТранспортСБригадой = ТранспортныйОтделВызовСервера.НоваяСтруктураОтбора(Выборка, Выборка.Транспорт);
...
&НаСервере
Процедура БригадаЗакрепленнаяЗаТранспортом(Дата)
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрСведений.СоставБригады.СрезПоследних(, Период >= &НачалоПериода И Транспорт = &Транспорт) КАК СоставБригадыСрезПоследних";
