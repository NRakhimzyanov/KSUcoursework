...
Процедура ОбработкаПроведения(Отказ,Режим)
	...
	ВыборкаДетальныеЗаписи = ДанныеДвижений(Отказ);
	ЗаписатьДвижения(ВыборкаДетальныеЗаписи);

КонецПроцедуры
	
Процедура ОбработкаЗаполнения(ДанныеЗаполнения,СтандартнаяОбработка)
	
	РезультатЗапроса = ДанныеЗаполнения(ДанныеЗаполнения);
	ЗаписатьДанныеЗаполнения(РезультатЗапроса);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ОбщегоНазначенияВызовСервера.ПроверитьУсловие(Отказ, Условие, "Поле Расстояние должно быть заполнено");	
	
	Условие = ДатаДоставки < ТекущаяДатаСеанса();
	ОбщегоНазначенияВызовСервера.ПроверитьУсловие(Отказ, Условие, "Дата плановой доставки не может быть в прошедшем периоде");		
	
	Условие = НЕ ЗначениеЗаполнено(Контрагент);
	ОбщегоНазначенияВызовСервера.ПроверитьУсловие(Отказ, Условие, "Поле Контрагент должно быть заполнено");			
	
	Условие = НЕ ЗначениеЗаполнено(КонтактноеЛицо);
	ОбщегоНазначенияВызовСервера.ПроверитьУсловие(Отказ, Условие, "Поле Контакное лицо должно быть заполнено");				
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ОбщегоНазначенияВызовСервера.ПроверитьУсловие(Отказ, Условие, "Поле Транспорт должно быть заполнено");				
	
	Условие = НЕ ЗначениеЗаполнено(Водитель);
	ОбщегоНазначенияВызовСервера.ПроверитьУсловие(Отказ, Условие, "Поле Водитель должно быть заполнено");				
		
	Условие = НЕ ЗначениеЗаполнено(ГрузчикПервый);
	ОбщегоНазначенияВызовСервера.ПроверитьУсловие(Отказ, Условие, "Поле ГрузчикПервый должно быть заполнено");				
	
	Условие = НЕ ЗначениеЗаполнено(ГрузчикВторой);
	ОбщегоНазначенияВызовСервера.ПроверитьУсловие(Отказ, Условие, "Поле ГрузчикВторой должно быть заполнено");				
